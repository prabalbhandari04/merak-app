...
const CategoryCard = props => {
    ...
    return (
        <TouchableOpacity style={styles.container} onPress={() => {         
            // props.refresh();
            props.isDelete? onAlert(props.id):props.moneyValue ? props.navigation.navigate(props.item.status == true ? "Transaction details" : "Details deleted", { item: props.item }) : props.onPress()
        }}>
            ...
            <View style={styles.money}>
                <Text style={{ fontSize: FONTSIZE.header2, fontWeight: '600', color: props.type == "-" ? '#FF6363' : "#2FA4FF" }}>{props.moneyValue ? props.type : ""}{formatMoney(props.moneyValue)} {props.moneyValue ? "NRS" : ""} </Text>
            </View>
        </TouchableOpacity>
    )
}
